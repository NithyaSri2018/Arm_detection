...
def load_models():
    ...
    try:
        ...
        ultrasound_model_path = current_dir.parent / 'backend' / 'output' / 'best_ultrasound_classifier.pth'
        
        if not ultrasound_model_path.exists():
            raise FileNotFoundError(f"Ultrasound model file not found at {ultrasound_model_path}")
        
        ultrasound_model.load_state_dict(torch.load(ultrasound_model_path, map_location=torch.device('cpu')))
        ultrasound_model.eval()
        
        # Load ARM Classifier
        arm_model = ARMClassifier()
        # Adjust path for backend structure
        arm_model_path = current_dir.parent / 'backend' / 'output' / 'best_arm_classifier.pth'
        
        if not arm_model_path.exists():
            raise FileNotFoundError(f"ARM model file not found at {arm_model_path}")
        
        arm_model.load_state_dict(torch.load(arm_model_path, map_location=torch.device('cpu')))
        arm_model.eval()
